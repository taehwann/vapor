...
void CpuPressureSolver::prepareSystem(int n) {
...
ProjectionResult CpuPressureSolver::project(MacGridState& state, const ProjectionOptions& options) {
    if (linearSolver_ == &defaultSolver_) defaultSolver_.setOmega(options.relaxation);
    ...
    MacGridOperators::computeDivergence(state, divergence_);
    ...
    result.linearSolve = linearSolver_->solve(system_, options.linearSolve);
    ...
    MacGridOperators::applyPressureGradient(state, system_.x, options.dt);
    MacGridOperators::enforceClosedBoundaries(state);
    result.divergenceAfter = MacGridOperators::maxDivergence(state);
    return result;
}

void CpuPressureSolver::reset() noexcept {
    std::fill(system_.x.begin(), system_.x.end(), 0.f);

...
void VolumeRenderer::init(const RenderData& data) {
    n = data.width;
...
void VolumeRenderer::upload(const RenderData& data) {
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_3D, volumeTex);
    if (data.densityBuffer) glMemoryBarrier(GL_PIXEL_BUFFER_BARRIER_BIT);
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, data.densityBuffer);
    glTexSubImage3D(GL_TEXTURE_3D, 0, 0, 0, 0,
        data.width, data.height, data.depth, GL_RED, GL_FLOAT,
        data.densityBuffer ? nullptr : data.density.data());
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);
}

void VolumeRenderer::updateDomain(const RenderData& data) {
    const float s = data.boxSize;
    float cube[] = {
        // -Z face
        0,0,0,  0,s,0,  s,s,0,  0,0,0,  s,s,0,  s,0,0,
...
void VolumeRenderer::render(const RenderData& data, int w, int h, const float* mvp, float cx, float cy, float cz, float stepScale, float alphaMul, float lightX, float lightY, float lightZ, float shadowStr, float shadowStep, FILE* logFile) {
    upload(data);
    glViewport(0, 0, w, h);

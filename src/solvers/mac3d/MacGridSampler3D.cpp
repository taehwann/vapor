...
float MacGridSampler3D::sampleCell(std::span<const float> f, Vec3 p) const {
...
float MacGridSampler3D::sampleVxField(std::span<const float> f, Vec3 p) const {
...
float MacGridSampler3D::sampleVyField(std::span<const float> f, Vec3 p) const {
...
float MacGridSampler3D::sampleVzField(std::span<const float> f, Vec3 p) const {
    const float gx = std::clamp(p.x / h() - 0.5f, 0.f, float(state_.resolution() - 1));
    const float gy = std::clamp(p.y / h() - 0.5f, 0.f, float(state_.resolution() - 1));
...
Vec3 MacGridSampler3D::velocity(Vec3 p, VelocityFieldView field) const {

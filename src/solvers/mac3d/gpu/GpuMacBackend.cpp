...
void GpuMacBackend::validateGrid(const MacGridState3D& state) const {
...
void GpuMacBackend::init(const MacGridState3D& state) {
...
void GpuMacBackend::project(float dt, int iterations, float omega, float hInv, float h,
    ...
    projectDevice(dt, iterations, omega, hInv, h);
...
void GpuMacBackend::projectDevice(float dt, int iterations, float omega, float hInv, float h) {
    ...
    for (int iter = 0; iter < iterations; ++iter) {
        ...
        glDispatchCompute((n + 63) / 64, (n + 1) / 2, (n + 1) / 2);
        glMemoryBarrier(GL_SHADER_STORAGE_BARRIER_BIT);
...
void GpuMacBackend::dispatchSL(int mode, int count, float dx, float dt, GLuint source, GLuint output) {
    glUseProgram(progAdvectSL);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 0, ssboVx);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 1, ssboVy);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 2, ssboVz);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 3, source ? source : ssboAdvectSrc);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 4, output ? output : ssboFwd);

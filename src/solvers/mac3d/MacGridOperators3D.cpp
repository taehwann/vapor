...
namespace {
float divergenceAt(const MacGridState3D& s, int x, int y, int z) {
...
void MacGridOperators3D::enforceClosedBoundaries(MacGridState3D& s) {
...
void MacGridOperators3D::computeDivergence(const MacGridState3D& s, std::span<float> output) {
...
float MacGridOperators3D::maxDivergence(const MacGridState3D& s) {
    float result = 0.f;
    const int n = s.resolution();
    for (int z = 0; z < n; ++z) for (int y = 0; y < n; ++y) for (int x = 0; x < n; ++x) {
...
void MacGridOperators3D::applyPressureGradient(MacGridState3D& s, std::span<const float> p, float dt) {

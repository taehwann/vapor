...
namespace {
// Transport and limiting read immutable source fields in every pass.
class Kernel {
public:
            ...
            for (int y = 0; y < state_.resolution(); ++y) {
                for (int x = 0; x < state_.resolution(); ++x) {
                    ...
                    if (posFwd.x < 0 || posFwd.x > state_.boxSize() || posFwd.y < 0 || posFwd.y > state_.boxSize() || posFwd.z < 0 || posFwd.z > state_.boxSize() ||
                        ...
                        std::sqrt(v.x * v.x + v.y * v.y + v.z * v.z) * dt / dx > cflMc) {
                        backScratch[i] = fwdScratch[i];
                        continue;
                    }
                    ...
                    // Keep every limiter stencil on the immutable old field.
                    // The reverse value at i is no longer needed after this.
                    backScratch[i] = std::clamp(corrected, fMin, fMax);
                }
            }
        }
        std::copy(backScratch.begin(), backScratch.begin() + N, field.begin());
    }
